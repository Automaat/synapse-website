...
Pick a type (`work`, `pet`, or a custom value). Optionally set setup commands and sandbox resources.
...
From the Projects page → **Delete** (confirm). Sybra:

1. Cancels any running agents on this project
2. Removes all worktrees
3. Deletes the bare clone
4. Removes the descriptor YAML

Tasks tied to the deleted project keep their `project_id` field but lose the link. They'll show up in the board without a project badge.

## Rename a project

GitHub repository renames break the `id` field. Fix in two steps:

1. In the project YAML, update `id` and `clone_url` to the new values
