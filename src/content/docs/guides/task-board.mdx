...
Press `/` (or click the filter bar). Filters stack with AND:

- **Tag**: `tag:backend`
- **Project**: `project:Automaat/sybra`
- **Type**: `type:debug`
- **Agent**: `agent:running`

![Filtered board](/screenshots/dark/task-board-filtered.png)

Filter state lives in the URL (`?tag=backend&type=debug`). Bookmark useful views.
...
2. Click **Start agent** (or the row shortcut)
3. Sybra creates a worktree, spawns the agent, streams events live
