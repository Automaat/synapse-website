...
Triggers can be gated by conditions — a list of `{field, operator, value}` tuples combined with AND. Fields reference task fields (`task.tags`, `task.status`, `task.project_id`, `task.agent_mode`) or PR fields (`pr.state`, `pr.author`).
...
- `default-feature` — for `task_type: normal`
- `default-debug` — for `task_type: debug`
- `default-research` — for `task_type: research`
- `renovate-fix` — triggered by `pr.event` on Renovate PRs with red CI
